...
class BoltzFormerTextDecoder(nn.Module):
    def __init__(
        ...
        # slice classification layers
        self.output_classifier = output_classifier
        if output_classifier:
            self.classifier_cross_attention_layers = nn.ModuleList()
            self.classifier_ffn_layers = nn.ModuleList()
        ...
        for _ in range(self.num_layers):
            ...
            # slice classification layers
            if output_classifier:
                self.classifier_cross_attention_layers.append(
                    CrossAttentionLayer(
                        d_model=hidden_dim,
                        nhead=nheads,
                        dropout=0.0,
                ...
                self.classifier_ffn_layers.append(
                    FFNLayer(
                        d_model=hidden_dim,
                        dim_feedforward=dim_feedforward,
                        dropout=0.0,
        ...
        if output_classifier:
            self.classifier_query = nn.Embedding(1, hidden_dim)
    ...
    def forward(
        ...
        query_embed = self.query_embed_.weight.unsqueeze(1).repeat(1, bs, 1)
        output = self.query_feat_.weight.unsqueeze(1).repeat(1, bs, 1)
        predictions_class = []
        predictions_mask = []
        sampled_patches = []
        predictions_captions = []

        cls_output = self.classifier_query.weight.unsqueeze(1).repeat(1, bs, 1)
        ...
        for i in range(self.num_layers):
            ...
            # FFN Layer
            cls_output = self.classifier_ffn_layers[i](cls_output)
    ...
    def forward_prediction_heads(
        ...
        if do_boltzmann:
            ...
            if print_mask_ratio:
                print(
                    f"Layer {layer_id}: Original ratio: {ori_ratio:.4f}, Boltzman ratio: {boltz_ratio:.4f}, Combined ratio: {combined_ratio:.4f}"
                )
